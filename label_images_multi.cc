...
{
  if (argc != 4) {
    fprintf(stderr, "label_images <tflite model1> <tflite model2> <dataset path>\n");
    return 1;
  }

  auto model_filename = argv[1];
  auto model_filename2 = argv[2];
  auto folder_path = std::string(argv[3]);
  ...
  std::unique_ptr<tflite::FlatBufferModel> model2 =
    tflite::FlatBufferModel::BuildFromFile(model_filename2);
  TFLITE_MINIMAL_CHECK(model != nullptr);
  ...
  // tasks so that the Interpreter can read the provided model.
  tflite::InterpreterBuilder builder(*model, resolver);
  tflite::InterpreterBuilder builder2(*model2, resolver);
  std::unique_ptr<tflite::Interpreter> interpreter;
  ...
  interpreter->SetExternalContext(kTfLiteEdgeTpuContext, edgetpu_context.get());
  interpreter->SetNumThreads(1);
  interpreter2->SetExternalContext(kTfLiteEdgeTpuContext, edgetpu_context.get());
  ...
  for (size_t i = 0; i < labels.size(); ++i) {
    ...
    for (auto image_full_path: images_full_path) {
      ...
      result = predictImage(interpreter, image_full_path);
      if (result != i) {
        failed += 1;
      ...
      printf("1:\n");
      auto result2 = predictImage(interpreter2, image_full_path);
      result2 = predictImage(interpreter2, image_full_path);
      if (result2 != i) {
        failed2 += 1;
        printf("1: Prediction: %s   actual label: %s", labels[result].c_str(), actual_label.c_str());
      }
    }
  }
